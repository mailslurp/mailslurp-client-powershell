...
function Initialize-UploadAttachmentOptions {
    ...
    Process {
        ...
        if ($Base64Contents -eq $null) {
            throw "invalid value for 'Base64Contents', 'Base64Contents' cannot be null."
        }

        if ($Base64Contents.length -lt 1) {
            throw "invalid value for 'Base64Contents', the character length must be great than or equal to 1."
        }
...
function ConvertFrom-JsonToUploadAttachmentOptions {
    ...
    Process {
        ...
        $AllProperties = ("contentId", "contentType", "filename", "base64Contents")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'base64Contents' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "base64Contents"))) {
            throw "Error! JSON cannot be serialized due to the required property 'base64Contents' missing."
        } else {
