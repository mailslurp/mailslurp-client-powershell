...
SentSmsDto<PSCustomObject>
#>

function Initialize-SentSmsDto {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: maislurp-client-powershell => SentSmsDto' | Write-Debug
        ...
        if ($ReplyToSid -eq $null) {
            throw "invalid value for 'ReplyToSid', 'ReplyToSid' cannot be null."
        }

        if ($ReplyToId -eq $null) {
            throw "invalid value for 'ReplyToId', 'ReplyToId' cannot be null."
        }
...
Convert from JSON to SentSmsDto<PSCustomObject>
...
SentSmsDto<PSCustomObject>
#>
function ConvertFrom-JsonToSentSmsDto {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: maislurp-client-powershell => SentSmsDto' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in SentSmsDto
        $AllProperties = ("id", "userId", "phoneNumber", "fromNumber", "toNumber", "body", "sid", "replyToSid", "replyToId", "createdAt", "updatedAt")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "replyToSid"))) {
            throw "Error! JSON cannot be serialized due to the required property 'replyToSid' missing."
        } else {
            $ReplyToSid = $JsonParameters.PSobject.Properties["replyToSid"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "replyToId"))) {
            throw "Error! JSON cannot be serialized due to the required property 'replyToId' missing."
        } else {
