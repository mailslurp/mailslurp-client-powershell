...
# Get all Contact Groups in paginated format
try {
     $Result = Get-AllGroups -Before $Before -Page $Page -Since $Since -Size $Size -Sort $Sort
} catch {
...
# Get group and paginated contacts belonging to it
try {
     $Result = Get-GroupWithContactsPaginated -GroupId $GroupId -Before $Before -Page $Page -Since $Since -Size $Size -Sort $Sort
} catch {
