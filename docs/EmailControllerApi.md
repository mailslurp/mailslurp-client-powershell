...
[**Invoke-CheckEmailAudit1**](EmailControllerApi#Invoke-CheckEmailAudit1) | **POST** /emails/{emailId}/check-email-audit | Run aggregate email audit for a stored email
...
[**New-EmailAuditForEmail**](EmailControllerApi#New-EmailAuditForEmail) | **POST** /emails/{emailId}/audit | Persist aggregate email audit for a stored email
...
<a name="Invoke-CheckEmailAudit1"></a>
# **Invoke-CheckEmailAudit1**
> EmailAuditAnalysisResult Invoke-CheckEmailAudit1<br>
> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[-EmailId] <PSCustomObject><br>
...
# Run aggregate email audit for a stored email
try {
     $Result = Invoke-CheckEmailAudit1 -EmailId $EmailId
} catch {
    Write-Host ("Exception occured when calling Invoke-CheckEmailAudit1: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
...
<a name="New-EmailAuditForEmail"></a>
# **New-EmailAuditForEmail**
> EmailAuditDto New-EmailAuditForEmail<br>
> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[-EmailId] <PSCustomObject><br>
...
# Persist aggregate email audit for a stored email
try {
     $Result = New-EmailAuditForEmail -EmailId $EmailId
} catch {
    Write-Host ("Exception occured when calling New-EmailAuditForEmail: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
