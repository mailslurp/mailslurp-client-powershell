...
$TeamAccess = true # Boolean | DEPRECATED. Optionally filter by team access. (optional) (default to $false)

# List All Inboxes Paginated
try {
     $Result = Get-AllInboxes -Before $Before -Favourite $Favourite -Page $Page -Search $Search -Since $Since -Size $Size -Sort $Sort -Tag $Tag -TeamAccess $TeamAccess
} catch {
...
$DelayTimeout = 987 # Int64 | delayTimeout (optional)
...
$UnreadOnly = true # Boolean | unreadOnly (optional)

# Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
try {
     $Result = Get-Emails -InboxId $InboxId -Before $Before -DelayTimeout $DelayTimeout -Limit $Limit -MinCount $MinCount -RetryTimeout $RetryTimeout -Since $Since -Size $Size -Sort $Sort -UnreadOnly $UnreadOnly
} catch {
...
# Get inbox emails paginated
try {
     $Result = Get-InboxEmailsPaginated -InboxId $InboxId -Before $Before -Page $Page -Since $Since -Size $Size -Sort $Sort
} catch {
...
# Get Inbox Sent Emails
try {
     $Result = Get-InboxSentEmails -InboxId $InboxId -Before $Before -Page $Page -SearchFilter $SearchFilter -Since $Since -Size $Size -Sort $Sort
} catch {
...
# List Inboxes and email addresses
try {
     $Result = Get-Inboxes -Before $Before -Since $Since -Size $Size -Sort $Sort
} catch {
...
# List Organization Inboxes Paginated
try {
     $Result = Get-OrganizationInboxes -Before $Before -Page $Page -SearchFilter $SearchFilter -Since $Since -Size $Size -Sort $Sort
} catch {
...
# List inbox rulesets
try {
     $Result = Invoke-ListInboxRulesets -InboxId $InboxId -Before $Before -Page $Page -SearchFilter $SearchFilter -Since $Since -Size $Size -Sort $Sort
} catch {
...
# List inbox tracking pixels
try {
     $Result = Invoke-ListInboxTrackingPixels -InboxId $InboxId -Before $Before -Page $Page -SearchFilter $SearchFilter -Since $Since -Size $Size -Sort $Sort
} catch {
