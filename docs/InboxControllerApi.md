...
# Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
try {
     $Result = Get-Emails -InboxId $InboxId -DelayTimeout $DelayTimeout -Limit $Limit -MinCount $MinCount -RetryTimeout $RetryTimeout -Since $Since -Size $Size -Sort $Sort -UnreadOnly $UnreadOnly
} catch {
...
# Get Inbox Sent Emails
try {
     $Result = Get-InboxSentEmails -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# List Organization Inboxes Paginated
try {
     $Result = Get-OrganizationInboxes -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# List inbox rulesets
try {
     $Result = Invoke-ListInboxRulesets -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# List inbox tracking pixels
try {
     $Result = Invoke-ListInboxTrackingPixels -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
