...
$EmailAddress = "EmailAddress_example" # String | emailAddress (optional)
$ExpiresAt = Get-Date # System.DateTime | expiresAt (optional)
$ExpiresIn = 987 # Int64 | expiresIn (optional)
$Favourite = true # Boolean | favourite (optional)
$InboxType = "HTTP_INBOX" # String | inboxType (optional)
$Name = "Name_example" # String | name (optional)
$Tags = @("Inner_example") # String[] | tags (optional)
$UseDomainPool = true # Boolean | useDomainPool (optional)

# Create new random inbox
try {
     $Result = New-NewEmailAddress -AllowTeamAccess $AllowTeamAccess -Description $Description -EmailAddress $EmailAddress -ExpiresAt $ExpiresAt -ExpiresIn $ExpiresIn -Favourite $Favourite -InboxType $InboxType -Name $Name -Tags $Tags -UseDomainPool $UseDomainPool
} catch {
...
# Create new random inbox
try {
     $Result = New-NewEmailAddress1 -AllowTeamAccess $AllowTeamAccess -Description $Description -EmailAddress $EmailAddress -ExpiresAt $ExpiresAt -ExpiresIn $ExpiresIn -Favourite $Favourite -InboxType $InboxType -Name $Name -Tags $Tags -UseDomainPool $UseDomainPool
} catch {
