...
$WaitForConditions = (Initialize-WaitForConditions -Count 123 -CountType "EXACTLY" -InboxId "InboxId_example" -VarMatches @((Initialize-MatchOption -Field "SUBJECT" -Should "CONTAIN" -Value "Value_example")) -SortDirection "ASC" -Timeout 123 -UnreadOnly $false) # WaitForConditions | Conditions to apply to emails that you are waiting for (optional)
...
# Wait or return list of emails that match simple matching patterns
try {
     $Result = Wait-ForMatchingEmail -MatchOptions $MatchOptions -Count $Count -Delay $Delay -InboxId $InboxId -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
