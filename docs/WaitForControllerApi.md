...
$WaitForConditions = (Initialize-WaitForConditions -Count 123 -CountType "EXACTLY" -InboxId "InboxId_example" -VarMatches @((Initialize-MatchOption -Field "SUBJECT" -Should "CONTAIN" -Value "Value_example")) -Since Get-Date -SortDirection "ASC" -Timeout 123 -UnreadOnly $false) # WaitForConditions | Conditions to apply to emails that you are waiting for (optional)
...
# Wait for and return count number of emails. Hold connection until inbox count matches expected or timeout occurs
try {
     $Result = Wait-ForEmailCount -Count $Count -Delay $Delay -InboxId $InboxId -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
...
# Fetch inbox's latest email or if empty wait for an email to arrive
try {
     $Result = Wait-ForLatestEmail -Delay $Delay -InboxId $InboxId -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
...
# Wait or return list of emails that match simple matching patterns
try {
     $Result = Wait-ForMatchingEmails -MatchOptions $MatchOptions -Count $Count -Delay $Delay -InboxId $InboxId -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
 ...
 **Count** | **Int32**| Number of emails to wait for. Must be greater or equal to 1 | [optional] 
 **Delay** | **Int64**| Max milliseconds delay between calls | [optional] 
 **InboxId** | [**String**](String)| Id of the inbox we are fetching emails from | [optional] 
 **Since** | **System.DateTime**| Filter for emails that were received after the given timestamp | [optional] 
 **Sort** | **String**| Sort direction | [optional] 
...
# Wait for or return the first email that matches provided MatchOptions array
try {
     $Result = Wait-ForMatchingFirstEmail -MatchOptions $MatchOptions -Delay $Delay -InboxId $InboxId -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
...
# Wait for or fetch the email with a given index in the inbox specified. If index doesn't exist waits for it to exist or timeout to occur.
try {
     $Result = Wait-ForNthEmail -Delay $Delay -InboxId $InboxId -Index $Index -Since $Since -Sort $Sort -Timeout $Timeout -UnreadOnly $UnreadOnly
} catch {
