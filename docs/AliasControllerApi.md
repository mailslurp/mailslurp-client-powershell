...
$CreateAliasOptions = (Initialize-CreateAliasOptions -EmailAddress "EmailAddress_example" -InboxId "InboxId_example" -Name "Name_example" -UseThreads $false) # CreateAliasOptions | 
...
# Get all email aliases you have created
try {
     $Result = Get-Aliases -Page $Page -Size $Size -Sort $Sort -Since $Since -Before $Before
} catch {
...
$ReplyToAliasEmailOptions = (Initialize-ReplyToAliasEmailOptions -Body "Body_example" -IsHTML $false -Charset "Charset_example" -Attachments @("Attachments_example") -TemplateVariables "TODO" -Template "Template_example" -SendStrategy "SINGLE_MESSAGE" -UseInboxName $false -Html $false) # ReplyToAliasEmailOptions | 
