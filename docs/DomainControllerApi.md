...
[**Get-AvailableDomainRegions**](DomainControllerApi#Get-AvailableDomainRegions) | **GET** /domains/available-domain-regions | Get all usable domains with account region status
...
<a name="Get-AvailableDomainRegions"></a>
# **Get-AvailableDomainRegions**
> DomainRegionGroupsDto Get-AvailableDomainRegions<br>
> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[-InboxType] <String><br>
...
# Get all usable domains with account region status
try {
     $Result = Get-AvailableDomainRegions -InboxType $InboxType
} catch {
    Write-Host ("Exception occured when calling Get-AvailableDomainRegions: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
...
[**DomainRegionGroupsDto**](DomainRegionGroupsDto) (PSCustomObject)
