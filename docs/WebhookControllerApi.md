...
# Get results for all webhooks
try {
     $Result = Get-AllWebhookResults -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# List Webhooks Paginated
try {
     $Result = Get-AllWebhooks -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# Get paginated webhooks for an Inbox
try {
     $Result = Get-InboxWebhooksPaginated -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# **Get-WebhookResult**
> WebhookResultEntity Get-WebhookResult<br>
...
[**WebhookResultEntity**](WebhookResultEntity) (PSCustomObject)
...
# Get a webhook results for a webhook
try {
     $Result = Get-WebhookResults -WebhookId $WebhookId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
