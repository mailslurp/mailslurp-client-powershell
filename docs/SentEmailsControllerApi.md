...
------------- | ------------- | -------------
[**Get-AllSentTrackingPixels**](SentEmailsControllerApi#Get-AllSentTrackingPixels) | **GET** /sent/tracking-pixels | Get all sent email tracking pixels in paginated form
[**Get-SentEmail**](SentEmailsControllerApi#Get-SentEmail) | **GET** /sent/{id} | Get sent email receipt
[**Get-SentEmailTrackingPixels**](SentEmailsControllerApi#Get-SentEmailTrackingPixels) | **GET** /sent/{id}/tracking-pixels | Get all tracking pixels for a sent email in paginated form
...
# Get all sent email tracking pixels in paginated form
try {
     $Result = Get-AllSentTrackingPixels -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# Get all tracking pixels for a sent email in paginated form
try {
     $Result = Get-SentEmailTrackingPixels -Id $Id -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# Get all sent emails in paginated form
try {
     $Result = Get-SentEmails -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
...
# Get all sent organization emails in paginated form
try {
     $Result = Get-SentOrganizationEmails -InboxId $InboxId -Page $Page -SearchFilter $SearchFilter -Size $Size -Sort $Sort
} catch {
